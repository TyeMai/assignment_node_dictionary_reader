...
var messages = { //step one is to load a dictionary,
  ...
  loadingStep: () => {
    ...
    load.finder().then((name) => { // searches for and then loads a dictionry
      ...
      var onData = (data) => { //this function handels the instream for step 1 selecting and loading a dic.
        ...
        if (data === 'q') {
          ...
          var dicStats = load.blowLoad(dicName) //loads(creates stream) the dictioanry by name.
  ...
  //step 3 asks user for a word to serach for. then displays any matches + defination
    ...
    var onTermRecieved = (data) => {
      ...
      if ((/[^a-zA-Z]/.test(data))) {
        ...
        searcher.search(choice, data).then(term => {
          ...
          term.forEach(match => {
            console.log(match)
          //  console.log(load.selectedDic[match])
          })
          if (term.length === 0) {
